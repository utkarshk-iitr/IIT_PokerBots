...
_SUITS  = 'cdhs'
_ALL_EVAL7_CARDS = [eval7.Card(r + s) for r in _RANKS for s in _SUITS]
...
def _board_texture_score(board: list) -> float:
    ...
    high = sum(1 for r in ranks if r >= 11)
...
class OppBidTracker:
    ABS_BIDDING_EPSILON  = 2
    PCT_BIDDING_EPSILON  = 0.10
    REASONABLE_DIST      = 10
    ...
    def adapt_bid(self, default_bid: int, pot: int, stack: int) -> int:
        ...
        if self.is_excessive:
            # ═══ FIX 5: Anti-extreme-bidder — match or exceed ═══
            # Against binary bidders (0 or 1245+), we must outbid them
    ...
    @property
    def is_binary_bidder(self) -> bool:
        """Detect binary bidders (bid 0 or near-stack)."""
        if self.bid_count < 3:
            return False
        low_bids = sum(1 for b in self._bids if b <= 5)
        high_bids = sum(1 for b in self._bids if b >= STARTING_STACK * 0.20)
        return (low_bids + high_bids) >= self.bid_count * 0.80

    @property
    def is_high_bidder(self) -> bool:
        """Detect consistently high bidders (avg bid > 80)."""
        if self.bid_count < 3:
            return False
        return self.avg_bid > 80
...
class Player(BaseBot):
    """
    SneakBot v4 — Hardened with 7 critical fixes.
    """
    ...
    def __init__(self):
        ...
        # ═══ FIX 3: Track bet sizes for randomization seed ═══
        self._bet_size_seed: int = 0
    ...
    # ─────────────────────────────────────────────────────────────────────
    # ═══ FIX 3: Randomized bet sizing helper ═══
    # ─────────────────────────────────────────────────────────────────────
    def _random_bet_size(self, pot: int, base_frac: float,
                         street: str) -> int:
        """
        Returns a randomized bet size around base_frac * pot.
        Varies between 0.7x and 1.4x the base to prevent pattern detection.
        """
        # Pick from several sizing tiers to be unpredictable
        jitter = random.choice([0.70, 0.85, 1.0, 1.15, 1.40])
        size = int(pot * base_frac * jitter)
        return max(BIG_BLIND, size)

    # ─────────────────────────────────────────────────────────────────────
    # ═══ FIX 1: Universal big-pot protection ═══
    # ─────────────────────────────────────────────────────────────────────
    def _big_pot_protection(self, s: PokerState, equity: float,
                            cost: int, pot: int) -> bool:
        """
        Returns True if we should FOLD to protect against catastrophic losses.
        Active on ALL streets (flop, turn, river).

        Key thresholds:
          - cost > 40% of stack AND equity < 0.75 → fold
          - cost > 25% of stack AND equity < 0.65 → fold
          - cost > pot * 2.0 (overbet) AND equity < 0.80 → fold
          - River: cost > 60% stack AND equity < 0.70 → fold
        """
        if cost <= 0:
            return False
        if not s.can_act(ActionFold):
            return False

        commit_ratio = cost / max(s.my_chips, 1)
        bet_to_pot   = cost / max(pot, 1)

        # Absolute stack protection — never risk >40% stack without premium
        if commit_ratio > 0.40 and equity < 0.75:
            return True

        # Moderate stack risk — need solid hand
        if commit_ratio > 0.25 and equity < 0.65:
            return True

        # Overbet detection — opponent bets 2x+ pot, they're very strong
        if bet_to_pot > 2.0 and equity < 0.80:
            return True

        # Large overbet — opponent bets 1.5x+ pot
        if bet_to_pot > 1.5 and equity < 0.72:
            return True

        # Pot-sized bet on later streets
        if s.street in ('turn', 'river') and bet_to_pot > 1.0 and equity < 0.65:
            return True

        # River-specific tighter protection
        if s.street == 'river':
            if commit_ratio > 0.20 and equity < 0.58:
                return True
    ...
    # ─────────────────────────────────────────────────────────────────────
    def _discount_equity_for_bet_size(self, equity: float, cost: int,
                                       pot: int, street: str) -> float:
        """
        When opponent bets large, their range is stronger than random.
        Discount our MC equity (which assumes random opponent) proportionally.
        Larger bets → larger discounts.
        """
        if cost <= 0:
            return equity

        bet_frac = cost / max(pot, 1)

        # Overbet > 2x pot — very strong signal
        if bet_frac > 2.0:
            equity *= 0.65
        # Overbet > 1.5x pot
        elif bet_frac > 1.5:
        ...
        # Half-pot+ bet
            ...
            equity *= 0.88
        # Small bet — minor discount
        elif bet_frac > 0.25:
            equity *= 0.95

        # Later streets carry stronger signals
        if street == 'river' and bet_frac > 0.5:
            equity *= 0.92
        elif street == 'turn' and bet_frac > 0.75:
            equity *= 0.95

        return max(0.0, equity)

    # ─────────────────────────────────────────────────────────────────────
    # ═══ FIX 7: Re-raise cap check ═══
    # ─────────────────────────────────────────────────────────────────────
    def _should_cap_reraise(self, s: PokerState, equity: float,
                             cost: int) -> bool:
        """
        Returns True if we should NOT re-raise (just call or fold instead).
        Prevents runaway escalation with marginal hands.

        If opponent has already raised (their wager > our wager + 3BB),
        we need a much stronger hand to re-raise.
        """
        if cost <= 0:
            return False

        # Detect we're facing a raise (opponent's wager significantly exceeds ours)
        facing_raise = s.opp_wager > s.my_wager + BIG_BLIND * 2

        if facing_raise:
            # Only re-raise with very strong equity
            if equity < 0.78:
                return True  # Don't re-raise, just call or fold

        # Never re-raise when we've already committed > 20% of stack
        # and equity is below premium
        total_committed = STARTING_STACK - s.my_chips
        if total_committed > STARTING_STACK * 0.20 and equity < 0.75:
            return True
    ...
    def on_hand_start(self, game_info: GameInfo,
        ...
        self._bet_size_seed = random.randint(0, 1000)
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Auction strategy  (MIT formula + FIX 5 anti-extreme-bidder)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _auction_action(self, game_info: GameInfo, s: PokerState):
        ...
        equity_diff = max(0.01, min(equity_diff, 0.40))
        ...
        # ═══ FIX 5: Anti-extreme-bidder — adaptive floor bids ═══
        # Against binary bidders (0 or 1245), use a different strategy:
        # - With strong hands: bid very high to guarantee auction win
        # - With weak hands: bid 0 to concede cheaply
        if self._bid_tracker.is_binary_bidder:
            if equity >= 0.65:
                # Strong hand — must outbid their high bid (~1245)
                default_bid = max(default_bid, int(stack * 0.30))
            elif equity >= 0.55:
                default_bid = max(default_bid, int(stack * 0.20))
            else:
                # Weak hand — concede auction, save chips
                default_bid = min(default_bid, 1)
        elif self._bid_tracker.is_high_bidder:
            # Against consistently high bidders (e.g. GnG at 95-287)
            if equity >= 0.60:
                default_bid = max(default_bid, int(self._bid_tracker.avg_bid * 1.15))
            elif equity >= 0.50:
                default_bid = max(default_bid, int(self._bid_tracker.avg_bid * 0.90))
            else:
                default_bid = min(default_bid, int(self._bid_tracker.avg_bid * 0.30))
        else:
            # Standard floor bids (same as before but slightly adjusted)
            if equity >= 0.72:
        ...
        # Ceiling — don't bid more than reasonable
        if not self._bid_tracker.is_binary_bidder:
            max_sensible = min(stack, max(int(pot * 0.50), int(stack * 0.25)))
            default_bid  = min(default_bid, max_sensible)
        else:
            # Against binary bidders, allow higher bids when we have strength
            max_sensible = min(stack, int(stack * 0.35))
            default_bid  = min(default_bid, max_sensible)
    ...
    def _preflop_action(self, game_info: GameInfo, s: PokerState):
        ...
        if cost > 0:
            ...
            # ═══ FIX 7: Pre-flop re-raise cap ═══
            # If opponent has already raised big, don't re-raise without premium
            facing_big_raise = cost > BIG_BLIND * 8

            if eq >= ELITE:
                return self._make_raise(s, s.opp_wager + max(cost * 4, BIG_BLIND * 10))
            if eq >= PREMIUM:
                if facing_big_raise:
                    # Against big raises, just call with premium (don't escalate)
                    if cost <= STARTING_STACK * 0.15:
                        return ActionCall()
                    return ActionFold()
    ...
    def _postflop_action(self, game_info: GameInfo, s: PokerState):
        ...
        # Range-weighted blend
        equity = self._blend_equity(mc_eq, s.my_hand, s.board,
                                    game_info.time_bank)
        pot  = s.pot
        cost = s.cost_to_call

        # ═══ FIX 2: Apply bet-size-aware equity discount ═══
        equity = self._discount_equity_for_bet_size(equity, cost, pot, s.street)

        # ═══ FIX 1: Universal big-pot protection (ALL streets) ═══
        if self._big_pot_protection(s, equity, cost, pot):
            return ActionFold()
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Post-flop: WE SEE OPPONENT'S CARD  (6 threat modes + FIX 4)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _postflop_with_info(self, s: PokerState, equity: float,
        ...
        # When visible card is "air"/"low_threat" but opponent bets/raises
        # aggressively, the HIDDEN card is likely compensating.
        # Upgrade threat level based on opponent's betting action.
        if threat in ('air', 'low_threat') and cost > 0:
            bet_frac = cost / max(pot, 1)
            if bet_frac > 1.0:
                # Huge bet with "air" card visible → hidden card is strong
                threat = 'strong'
                strong_frac = 0.50
            elif bet_frac > 0.5:
                threat = 'marginal'
                ...
                # Moderate bet — slight upgrade
                if threat == 'air':
                    threat = 'low_threat'
                    strong_frac = 0.20

        # ═══ FIX 7: Re-raise cap ═══
        reraise_capped = self._should_cap_reraise(s, equity, cost)

        # MODE 1: DANGEROUS
        if threat == 'dangerous':
            if cost > 0:
                if equity >= 0.78 and not reraise_capped:
                ...
                return ActionCall()
            if equity >= 0.78:
                return self._make_raise(s, self._random_bet_size(pot, 0.70, s.street))
            if equity >= 0.60:
                return ActionCheck()
            return ActionCheck()
        ...
        if threat == 'strong':
            if cost > 0:
                pot_odds = cost / max(pot + cost, 1)
                if equity >= 0.72 and not reraise_capped:
                ...
                return ActionCall()
            if equity >= 0.70:
                return self._make_raise(s, self._random_bet_size(pot, 0.65, s.street))
            if equity >= 0.55:
                if random.random() < 0.55:
                    return self._make_raise(s, self._random_bet_size(pot, 0.40, s.street))
                return ActionCheck()
            return ActionCheck()
        ...
        if threat in ('draw', 'draw_weak'):
            ...
            if cost > 0:
                pot_odds = cost / max(pot + cost, 1)
                if equity >= 0.62 and not reraise_capped:
                ...
                return ActionCall()
            if equity >= 0.52:
                bet = max(denial_bet, self._random_bet_size(pot, 0.55, s.street))
                return self._make_raise(s, bet)
        ...
        if threat == 'marginal':
            if cost > 0:
                pot_odds = cost / max(pot + cost, 1)
                if equity >= 0.68 and not reraise_capped:
                    return self._make_raise(s, int(pot * 0.65 + cost))
                if equity >= 0.48:
                    if random.random() < 0.15 and not reraise_capped:
                ...
                return ActionCall()
            if equity >= 0.65:
                return self._make_raise(s, self._random_bet_size(pot, 0.60, s.street))
            if equity >= 0.48:
            ...
            if equity >= 0.32 and s.street in ('flop', 'turn'):
                if random.random() < 0.35:
                    return self._make_raise(s, self._random_bet_size(pot, 0.30, s.street))
            return ActionCheck()
        ...
        if threat == 'low_threat':
            if cost > 0:
                pot_odds = cost / max(pot + cost, 1)
                if equity >= 0.65 and not reraise_capped:
            ...
            if equity >= 0.42:
                if random.random() < 0.60:
                    return self._make_raise(s, self._random_bet_size(pot, 0.45, s.street))
                return ActionCheck()
            if s.street in ('flop', 'turn') and random.random() < 0.35:
                return self._make_raise(s, self._random_bet_size(pot, 0.35, s.street))
            return ActionCheck()

        # MODE 6: AIR — exploitation (with FIX 4 awareness)
        if cost > 0:
            pot_odds = cost / max(pot + cost, 1)
            if equity >= 0.58 and not reraise_capped:
                return self._make_raise(s, int(pot * 0.80 + cost))
            if equity >= 0.42:
                # ═══ FIX 7: Don't re-raise with marginal hands vs air ═══
                if random.random() < 0.15 and not reraise_capped:
                    return self._make_raise(s, int(pot * 0.50 + cost))
                return ActionCall()
        ...
        # No bet to face — pressure vs air (FIX 3: randomized sizing)
        if equity >= 0.52:
            base = 0.75 if s.street == 'river' else 0.55
            return self._make_raise(s, self._random_bet_size(pot, base, s.street))
        if equity >= 0.32:
            if s.street in ('flop', 'turn'):
                return self._make_raise(s, self._random_bet_size(pot, 0.45, s.street))
            if s.street == 'river' and random.random() < 0.50:
                return self._make_raise(s, self._random_bet_size(pot, 0.55, s.street))
            return ActionCheck()
        ...
        if s.street == 'turn':
            ...
            if random.random() < barrel:
                return self._make_raise(s, self._random_bet_size(pot, 0.50, s.street))
        if s.street == 'river':
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Post-flop: OPPONENT HAS INFO  (FIX 6: tighter calling)
    # ─────────────────────────────────────────────────────────────────────

    def _postflop_vs_info(self, s: PokerState, equity: float,
                          pot: int, cost: int):
        """
        ═══ FIX 6: Much tighter calling when opponent has our card info ═══

        When opponent won the auction, they KNOW one of our cards.
        They will only bet when they're confident — so we must fold more.
        Key changes:
          - Tighter call threshold: pot_odds + 0.18 (was 0.12)
          - River protection is stricter
          - Reduced bluffing frequency (opponent can read us)
          - Higher equity needed to raise
        """
        # ═══ FIX 6: Stricter stack protection when opponent has info ═══
        if cost > 0:
            commit = cost / max(s.my_chips, 1)
            bet_to_pot = cost / max(pot, 1)

            # River: very tight — opponent knows our hand, their bet is informed
            if s.street == 'river':
                if commit > 0.30 and equity < 0.65:
                    if s.can_act(ActionFold):
                        return ActionFold()
                if commit > 0.15 and equity < 0.55:
                    if s.can_act(ActionFold):
                        return ActionFold()

            # Turn: opponent knows our hand, large bets are very credible
            if s.street == 'turn':
                if commit > 0.35 and equity < 0.65:
                    if s.can_act(ActionFold):
                        return ActionFold()
                if bet_to_pot > 1.5 and equity < 0.70:
                    if s.can_act(ActionFold):
                        return ActionFold()

            # Flop: opponent might bluff with info, be somewhat less tight
            if s.street == 'flop':
                if commit > 0.40 and equity < 0.70:
                    if s.can_act(ActionFold):
                        return ActionFold()
        ...
        if cost > 0:
            ...
            if equity >= 0.80 and not reraise_capped:
                return self._make_raise(s, int(pot * 0.65 + cost))
            if equity >= 0.62:
                return ActionCall()
            # ═══ FIX 6: Tighter call buffer (was +0.12, now +0.18) ═══
        ...
        # No bet to face — be cautious, opponent might be trapping
        if equity >= 0.80:
            return self._make_raise(s, self._random_bet_size(pot, 0.55, s.street))
        if equity >= 0.65:
            if random.random() < 0.30:
                return self._make_raise(s, self._random_bet_size(pot, 0.35, s.street))
            return ActionCheck()
        # ═══ FIX 6: Reduced bluffing frequency — opponent can read us ═══
        if equity >= 0.55 and random.random() < 0.05:
            return self._make_raise(s, self._random_bet_size(pot, 0.25, s.street))
        return ActionCheck()

    # ─────────────────────────────────────────────────────────────────────
    # Post-flop: NO INFO ADVANTAGE
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _postflop_neutral(self, s: PokerState, equity: float,
        ...
        if cost > 0:
            pot_odds = cost / max(pot + cost, 1)
            if equity >= 0.70 and not reraise_capped:
                return self._make_raise(s, int(pot * 0.70 + cost))
            if equity >= 0.55:
                if random.random() < 0.15 and not reraise_capped:
                    return self._make_raise(s, int(pot * 0.50 + cost))
